/**
 * Insert / remove / delete (and reparent) on the chapterly-api-server node
 * routes. Ported from chat-server-js/tests/node-edit-ops.test.js.
 * Each test starts from a fresh in-memory SQLite database.
 */
...
async function createChat(title = 'edit-ops'): Promise<string> {
  const res = await request(app).post('/api/chats').send({ title });
  expect(res.status).toBe(201);
  return res.body.id;
}

async function addNode(chatId: string, body: Record<string, unknown>): Promise<Record<string, unknown>> {
  const res = await request(app).post(`/api/chats/${chatId}/nodes`).send(body);
  expect(res.status).toBe(201);
  return res.body;
}

async function listNodes(chatId: string): Promise<Record<string, unknown>[]> {
  const res = await request(app).get(`/api/chats/${chatId}/nodes`);
  expect(res.status).toBe(200);
  return res.body;
}

async function linear(chatId: string) {
...
function ids(nodes: Record<string, unknown>[]): unknown[] {
...
describe('DELETE /api/chats/:chatId/nodes/:nodeId', () => {
  ...
  test('cascade deletes a mid-tree node and the whole descendant subtree', async () => {
    const chatId = await createChat();
    const t = await linear(chatId);
