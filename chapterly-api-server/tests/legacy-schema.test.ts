...
beforeAll(async () => {
  ...
  // Create the DB with the CURRENT schema first, then simulate a legacy DB
  // that gained an extra column (older productions kept `total_cost`).
  store = new SqlitePersistence(dbFile);
  await store.init();
  const legacy = new Database(dbFile);
  legacy.exec('ALTER TABLE chat_nodes ADD COLUMN total_cost REAL');
  legacy.close();
