...
export class IdbChatApiService implements ChatApiPort {
  ...
  async getProjects(): Promise<Project[]> {
    return this.tx(['projects'], 'readonly', tx => this.all<Project>(tx.objectStore('projects')));
  }
  ...
  async deleteProject(id: string, deleteChats = false): Promise<void> {
    await this.tx(['projects', 'chats', 'nodes'], 'readwrite', async tx => {
      ...
      const mine = (await this.all<Chat>(tx.objectStore('chats'))).filter(c => c.projectId === id);
  ...
  async getChats(): Promise<Chat[]> {
    const list = await this.tx(['chats'], 'readonly', tx => this.all<Chat>(tx.objectStore('chats')));
  ...
  async createNode(chatId: string, data: CreateNodeRequest): Promise<ChatNode> {
    const row: ChatNode = {
      id: this.id(), chatId, parentId: data.parentId ?? null, role: data.role,
      content: data.content ?? '', thinking: data.thinking ?? null,
  ...
  async editAssistant(chatId: string, nodeId: string, content: string, attachments?: NodeAttachment[], thinking?: string): Promise<ChatNode> {
    return this.editNodeVersion(nodeId, 'assistant', { content, attachments, thinking });
  }

  async editUser(chatId: string, nodeId: string, content: string, attachments?: NodeAttachment[]): Promise<ChatNode> {
  ...
  async branchUser(chatId: string, nodeId: string, data: BranchQuestionRequest): Promise<ChatNode> {
    return this.tx(['nodes', 'chats'], 'readwrite', async tx => {
      ...
      const row: ChatNode = {
        id: this.id(), chatId: old.chatId, parentId: old.parentId, role: 'user',
        content: data.content, thinking: null,
        modelId: data.modelId ?? old.modelId, providerId: data.providerId ?? old.providerId,
        version: 1, previousVersionId: null, isCurrent: true,
        createdAt: this.now(), updatedAt: this.now(),
  ...
  async createTopic(data: CreateTopicRequest): Promise<Topic> {
    const row: Topic = {
      id: this.id(), name: data.name, description: data.description ?? '', defaultModelId: data.defaultModelId ?? null,
      defaultSystemPrompt: data.defaultSystemPrompt ?? '', chatParametersId: data.chatParametersId ?? null,
      icon: data.icon ?? '', projectIds: data.projectIds ?? [], createdAt: this.now(), updatedAt: this.now()
    };
  ...
  async addProjectToTopic(topicId: string, projectId: string): Promise<Topic> {
    return this.updateTopic(topicId, { projectIds: [...new Set([...(await this.mustTopic(topicId)).projectIds, projectId])] } as UpdateTopicRequest);
  }
  async removeProjectFromTopic(topicId: string, projectId: string): Promise<Topic> {
    const t = await this.mustTopic(topicId);
    return this.updateTopic(topicId, { projectIds: t.projectIds.filter(id => id !== projectId) } as UpdateTopicRequest);
  }

  async getProviders(): Promise<ProviderConfig[]> {
    return this.tx(['providers'], 'readonly', tx => this.all<ProviderConfig>(tx.objectStore('providers')));
  }
  async createProvider(data: CreateProviderRequest): Promise<ProviderConfig> {
    const row: ProviderConfig = { id: this.id(), name: data.name, type: data.type, baseUrl: data.baseUrl, apiKey: data.apiKey, enabled: data.enabled ?? true };
    await this.tx(['providers'], 'readwrite', tx => this.req(tx.objectStore('providers').put(row)));
    return row;
  }
  ...
  async getModels(): Promise<ModelEntry[]> {
    return this.tx(['models'], 'readonly', tx => this.all<ModelEntry>(tx.objectStore('models')));
  }
  async createModel(data: CreateModelRequest): Promise<ModelEntry> {
    const row: ModelEntry = {
      id: this.id(), displayName: data.displayName, modelId: data.modelId, providerId: data.providerId,
      type: data.type, enabled: data.enabled ?? true, architecture: data.architecture,
      contextLength: (data as any).contextLength, description: (data as any).description, ...(data as any)
    } as ModelEntry;
    await this.tx(['models'], 'readwrite', tx => this.req(tx.objectStore('models').put(row)));
    return row;
  }
  ...
  async deleteModel(id: string): Promise<void> {
    await this.tx(['models'], 'readwrite', tx => this.req(tx.objectStore('models').delete(id)));
  }

  async getChatParameters(): Promise<ChatParameters[]> {
    return this.tx(['chatParameters'], 'readonly', tx => this.all<ChatParameters>(tx.objectStore('chatParameters')));
  }
  async getChatParameter(id: string): Promise<ChatParameters> {
    const row = await this.tx(['chatParameters'], 'readonly', tx => this.req<ChatParameters>(tx.objectStore('chatParameters').get(id)));
  ...
  async createChatParameters(data: ChatParametersDraft): Promise<ChatParameters> {
    ...
    const row: ChatParameters = {
      id: this.id(), name: data.name || '', temperature: data.temperature ?? null, topK: data.topK ?? null,
      topM: data.topM ?? null, topP: data.topM ?? null, stream: data.stream ?? null, thinking: data.thinking ?? null,
      thinkingLevel: data.thinkingLevel ?? null, reasoningEffort: data.thinkingLevel ?? null, createdAt: now, updatedAt: now
    };
    await this.tx(['chatParameters'], 'readwrite', tx => this.req(tx.objectStore('chatParameters').put(row)));
    return row;
  }
  async updateChatParameters(id: string, data: ChatParametersDraft): Promise<ChatParameters> {
    return this.tx(['chatParameters'], 'readwrite', async tx => {
      ...
      const next: ChatParameters = {
        ...row, name: data.name ?? row.name, temperature: data.temperature, topK: data.topK, topM: data.topM,
        topP: data.topM, stream: data.stream, thinking: data.thinking, thinkingLevel: data.thinkingLevel,
        reasoningEffort: data.thinkingLevel, updatedAt: this.now()
      };
      await this.req(store.put(next));
      return next;
    });
  ...
  async deleteChatParameters(id: string): Promise<void> {
    await this.tx(['chatParameters', 'projects', 'topics', 'chats', 'nodes', 'models'], 'readwrite', async tx => {
  ...
  private async clearOwnerParam(tx: IDBTransaction, storeName: StoreName, paramId: string): Promise<void> {
    ...
    for (const row of rows) {
      if (row.chatParametersId === paramId) await this.req(store.put({ ...row, chatParametersId: null }));
    }
  }
  ...
  private async editNodeVersion(nodeId: string, expectedRole: 'system' | 'user' | 'assistant', data: { content: string; attachments?: NodeAttachment[]; thinking?: string }): Promise<ChatNode> {
    return this.tx(['nodes', 'chats'], 'readwrite', async tx => {
      const store = tx.objectStore('nodes');
      const old = await this.req<ChatNode>(store.get(nodeId));
      if (!old) throw Object.assign(new Error('Node not found'), { status: 404 });
      ...
      await this.req(store.put(next));
      for (const child of children) await this.req(store.put({ ...child, parentId: next.id, updatedAt: this.now() }));
